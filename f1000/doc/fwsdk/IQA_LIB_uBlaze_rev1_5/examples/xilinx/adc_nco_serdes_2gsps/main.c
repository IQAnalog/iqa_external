...
int main ( int argc, FAR char *argv[] )
{
    ...
    uint32_t band_freq[] = { 0x10000000,    // 3.0G
                             0x0fbbbbbc,    // 2.95G
                             0x1199999a,    // 3.3G
                             0x12d3a06d,    // 3.53G
                             0x12666666,    // 3.45G
    ...
    status = iqa_adc_dsp_enable ( F1000_ID, ep_id, adc_id, BAND_1 | BAND_2, MODE_2GSPS, 1 );
    if ( status != IQA_SUCCESS )
    {
      printf ( "Failed to change ADC DSP rate mode or frequency index for EP-%c, ADC-%d, Band 1 & 2, status %d\r\n", 'A' + ep_id - 1, adc_id, status );
      return status;
    }
    ...
    printf ("\r\nPress space bar to switch to NCO freq index %d...\r\n", nco_index);
    wait_for_key(' ');

    status = iqa_adc_dsp_enable ( F1000_ID, ep_id, adc_id, BAND_1 | BAND_2, MODE_2GSPS, nco_index );
    if ( status != IQA_SUCCESS )
    {
      printf ( "Failed to change NCO freq index to %d, Band 1 & 2, status %d\r\n", nco_index, status );
      return status;
    }

    printf ("\r\nNCO freq index switched to %d successfully.\r\n", nco_index);
