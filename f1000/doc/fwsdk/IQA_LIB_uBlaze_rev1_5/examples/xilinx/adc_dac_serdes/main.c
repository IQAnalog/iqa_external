...
static int init_dac_dsp ( int f1000_id, int ep_id, int dac_id );
...
static int init_adc_dsp ( int f1000_id, int ep_id, int adc_id )
{
  ...
  status |= iqa_init_adc_dsp ( f1000_id, ep_id, adc_id, BAND_1, band_freq,
                               initial_phase, MODE_2GSPS, 7 );     // freq selected = 7
...
static int init_dac_dsp ( int f1000_id, int ep_id, int dac_id )
{
  ...
  // Default frequencies to be loaded into DAC DSP
  ...
  // 8 initial phase frequencies to be loaded into DAC DSP
  uint32_t initial_phase[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };

  status |= iqa_init_dac_dsp ( f1000_id, ep_id, dac_id, BAND_1, band_freq,
                               initial_phase, MODE_2GSPS, 7 );     // freq selected = 7
  status |= iqa_init_dac_dsp ( f1000_id, ep_id, dac_id, BAND_2, band_freq,
                               initial_phase, MODE_2GSPS, 7 );     // freq selected = 7
...
int main ( int argc, FAR char *argv[] )
{
    ...
    int         nco_index;          // Index into which NCO freq to use

    // Using 6GHz clock, EP-B, ADC-2, DAC-1
    fclk_mhz    = 6000;
    ep_id       = EP_B;
    adc_id      = ADC_2;
    dac_id      = DAC_1;

    // Create bitmap of which RX lanes (0, 1, 3, 6) to be enabled
    rx_lane_bitmap = (1 << 0) + (1 << 1) + (1 << 3) + (1 << 6);
    ...
    status = iqa_jesd_tx_link_enable ( F1000_ID, ep_id, 0 );

    if ( status != IQA_SUCCESS )
    {
      printf ( "Failed to Disable TX link in EP-%c, status %d\r\n", 'A' + ep_id - 1, status );
      return status;
    }
    ...
    status = init_adc_dsp ( F1000_ID, ep_id, dac_id );
    ...
    status = init_dac_dsp ( F1000_ID, ep_id, dac_id );

    if (status != IQA_SUCCESS)
    {
        printf("DAC DSP initialization failed.\r\n");
        return status;
    }


    //=================================
    // Set user defined ADC NCO Frequency Tuning Word
    //=================================

    // (48Msps) These are for 3.05G input freq with 2Gsps rate mode
    uint32_t adc_band_freq[] = { 0x10000000,    // 3.0G
                                 0x0fbbbbbc,    // 2.95G
                                 0x1199999a,    // 3.3G
                                 0x12d3a06d,    // 3.53G
                                 0x12666666,    // 3.45G
    ...
    // 8 initial phases to be loaded into DAC DSP
    uint32_t initial_phase[] = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0 };

    band_id = 1;

    // Skip frame sync until the last call to iqa_set_dac_dsp_freq()
    frame_sync = 0;

    status = iqa_set_adc_dsp_freq ( F1000_ID, ep_id, adc_id, band_id, adc_band_freq, initial_phase, frame_sync );
    if ( status != IQA_SUCCESS )
    {
    ...
    // Issue frame sync in the last call to iqa_set_dac_dsp_freq() so the freq take effect
    frame_sync = 1;
    ...
    //=================================
    // Set user defined DAC NCO Frequency Tuning Word
    //=================================

    // 64Msps
    uint32_t dac_band_freq[] = { 0x2400000,     //   9*2^22, 9*62.5M   = 562.5MHz
                                 0x3400000,     //  13*2^22, 13*62.5M  = 812.5MHz
                                 0x5400000,     //  21*2^22, 21*62.5M  = 1.3125GHz
                                 0x7c00000,     //  31*2^22, 31*62.5M  = 1.9375GHz
                                 0x8c00000,     //  35*2^22, 35*62.5M  = 2.1875GHz
                                 0x9c00000,     //  39*2^22, 39*62.5M  = 2.4375GHz
                                 0x10400000,    //  65*2^22, 65*62.5M  = 4.0625GHz
                                 0x41800000     // 262*2^22, 262*62.5M = 16.375GHz
                               };
    ...
    status = iqa_set_dac_dsp_freq ( F1000_ID, ep_id, dac_id, band_id, dac_band_freq, initial_phase, frame_sync );
    if ( status != IQA_SUCCESS )
    {
      printf ( "Failed to set DAC DSP frequency for EP-%c, DAC-%d, Band-%d, status %d\r\n", 'A' + ep_id - 1, dac_id, band_id, status );
      return status;
    }
    ...
    status = iqa_jesd_tx_link_enable ( F1000_ID, ep_id, 1 );

    if ( status != IQA_SUCCESS )
    {
      printf ( "Failed to Enable TX link in EP-%c, status %d\r\n", 'A' + ep_id - 1, status );
      return status;
    }

    printf ("\r\nLoad bit file into FPGA and establish link. Press space bar when done...\r\n");
    wait_for_key(' ');
    ...
    printf ( "\r\nADC -> SERDES TX, DAC <- SERDES RX %s\r\n", (status == IQA_SUCCESS ? "PASSED" : "FAILED") );

    //=================================
    // Call iqa_adc_dsp_enable() to
    // select different parameters
    //=================================

    // Switch to nco freq index 1
    nco_index = 1;

    printf ("\r\nPress space bar to switch to ADC NCO freq index %d...\r\n", nco_index);
    wait_for_key(' ');

    status = iqa_adc_dsp_enable ( F1000_ID, ep_id, adc_id, BAND_1 | BAND_2, MODE_2GSPS, nco_index );
    if ( status != IQA_SUCCESS )
    {
        printf ( "Failed to change ADC NCO freq index to %d, status %d\r\n", nco_index, status );
        return status;
    }

    printf ("ADC NCO freq index switched to %d successfully.\r\n", nco_index);

    // Switch to nco freq index 2
    nco_index = 2;
    ...
    printf ("\r\nPress space bar to switch to DAC NCO freq index %d...\r\n", nco_index);
    wait_for_key(' ');

    status = iqa_dac_dsp_enable ( F1000_ID, ep_id, dac_id, BAND_1 | BAND_2, MODE_2GSPS, nco_index );
    if ( status != IQA_SUCCESS )
    {
        printf ( "Failed to change DAC NCO freq index to %d, status %d\r\n", nco_index, status );
        return status;
    }
    printf ("DAC NCO freq index switched to %d successfully.\r\n", nco_index);
